...
local border = { { "▁", "None" }, { "▁", "None" }, { "▁", "None" }, { "▕", "NormalFloat" }, { "▔", "None" }, { "▔", "None" }, { "▔", "None" }, { "▏", "NormalFloat" } }
local M = {win=nil, ns=vim.api.nvim_create_namespace("mpv"), content_id=nil, title_id=nil}
local conf = {width=50, height=5, border= border, setup_widgets=false, timer={after=1000, throttle=250}}
local state = {thumbnail = nil, buf=nil, playing=false, jobid=nil, title=nil, paused=false, timing="", percent=0, muted=false, loaded=false, image=nil}
...
local by3 = function()
    local w = 0
    if state.image then
        w = conf.width - (state.image.rendered_geometry.width or 0)
        return (" "):rep(math.floor(w/4)-1)
    else
        return (" "):rep(math.floor(conf.width/4)-1)
    end
end
...
local refresh_screen = function()
    ...
    local image_width = 0
    if state.image then
        if state.image.rendered_geometry then
            image_width = state.image.rendered_geometry.width + 1
        end
    end
    local iw = (" "):rep(image_width)

    local dur = math.floor((state.percent/100) * conf.width)
    vim.api.nvim_buf_set_extmark(state.buf, M.ns, 0, 0, {
        virt_text = {
            {
    ...
    vim.api.nvim_buf_set_extmark(state.buf, M.ns, 0, 0, {
        virt_lines = {
            {
                {iw .. time1, hls.timer}, {(" "):rep(conf.width - 16 - image_width)}, {time2, hls.timer}
            },
            {
                {iw .. ("▁"):rep(dur), hls.progress},
                {char, hls.progress},
                {("▁"):rep(conf.width - dur - image_width), "Comment"}
            },
            {{"", ""}},
            {
                {iw .. by3() .."󰞓 ", hls.progress},
...
local function play_song(query)
    ...
    local image_width
    if state.thumbnail then
        -- require("image").from_url("https://img.youtube.com/vi/"..current_thumbnail.."/0.jpg", {}, function(image)
        -- require("image").from_url("https://img.youtube.com/vi/"..current_thumbnail.."/mqdefault.jpg", {}, function(image)
        require("image").from_url("https://i.ytimg.com/vi/"..state.thumbnail.."/mqdefault.jpg", {}, function(image)
            state.image = image
            local wo = vim.api.nvim_win_get_config(state.win)
            image:render({
                window = state.win,
                buffer = state.buf,
                x = wo.col, y = wo.row + 1,
            ...
            image_width = image.rendered_geometry.width + 1
            wo["width"] = wo["width"] + image_width
            image:move(wo.col - image_width, wo.row + 1)
            conf['width'] = conf['width'] + image_width
            vim.api.nvim_win_set_config(state.win, wo)
        end)
    end
    state.jobid = vim.fn.jobstart(command, {
        ...
        on_stdout = function(_, data)
            if data then
                ...
                if not data[1]:match("A:") and not data[1]:match("AO:") then
                    state.title = (" "):rep(image_width or 0) .. (data[1]:match([['(.*)']]) or state.title or "")
                end
        ...
        on_exit = function()
            state.playing = false
            state.title = nil
            if state.image then state.image:clear() end
            refresh_screen()
...
local function ask_input(query)
    ...
    local image_width = 0
    if state.image then image_width = state.image.rendered_geometry.width end
    M.title_id = vim.api.nvim_buf_set_extmark(
        state.buf,
        M.ns,
        0, 0,
        {
            virt_text = {
                { (" "):rep(image_width) .. 'Searching for "'.. query ..'"...', hls.title }
            },
            virt_text_pos = 'overlay',
            id = M.title_id
        }
	...
	if not query:match([[^https://.*youtube.com]]) then
		...
		if not vim.uv.fs_stat(query) then
			query = "ytdl://ytsearch:"..table.concat(vim.split(query, ' '), '+')
		end
    end

    state.thumbnail = query:match("v=([a-zA-Z0-9_]*)")
    ...
    if not state.playing then
        ...
        vim.notify("Song added to queue!")
        table.insert(queue, query)
    end
end

M.toggle_player = function()
    if state.loaded and vim.api.nvim_win_is_valid(state.win) then
    ...
    vim.bo[state.buf].filetype = 'mpv'
    state.win = vim.api.nvim_open_win(state.buf, true, win_opts)

    -- Title for mpv window
    M.title_id = vim.api.nvim_buf_set_extmark(state.buf, M.ns, 0, 0, {
        virt_text = {
            {state.title or (#queue >= 0 and queue[1] or 'Not Playing'), hls.title}
        }, virt_text_pos = 'overlay'
    })
    ...
    M.content_id = vim.api.nvim_buf_set_extmark(state.buf, M.ns, 0, 0, {
        virt_lines = {
